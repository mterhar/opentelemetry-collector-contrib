...
func (r *libhoneyReceiver) handleSomething(resp http.ResponseWriter, req *http.Request) {
	...
	otlpTraces, otlpLogs, err := toPsomething(dataset, simpleSpans, *r.cfg, *r.settings.Logger)
	if err != nil {
		errorutil.HTTPError(resp, err)
		return
	}

	numSpans := otlpTraces.SpanCount()
	if numSpans > 0 {
		ctx := r.obsreport.StartTracesOp(context.Background())
		err = r.nextTraces.ConsumeTraces(ctx, otlpTraces)
		r.obsreport.EndTracesOp(ctx, "protobuf", numSpans, err)
	}
	...
	if numLogs > 0 {
		...
		r.obsreport.EndLogsOp(ctx, "protobuf", numSpans, err)
	}
